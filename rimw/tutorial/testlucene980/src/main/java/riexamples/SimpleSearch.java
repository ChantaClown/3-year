...
public class SimpleSearch {
	 ...
	 * classes, creates and Index Searcher and search for documents which contain
	 * the word "probability" in the field "modelDescription" using the
	 * StandardAnalyzer Also contains and example sorting the results by reverse
	 * document number (index order). Also contains an example of a boolean
	 * programmatic query
	 * 
	 */
	public static void main(String[] args) {
		...
		// StoredFields is a reader of the stored fields
		System.out.println(
		...
		for (int i = 0; i < Math.min(10, topDocs.totalHits.value); i++) {
			try {
				System.out.println(topDocs.scoreDocs[i].doc + " -- score: " + topDocs.scoreDocs[i].score + " -- "
						+ storedFields.document(topDocs.scoreDocs[i].doc).get("modelDescription"));
